...
class UrbanChangeAid:
    ...
    def next_to_metrics(self):
        ...
        # Config sliders (já existente, mantenha)
        self.dialog.sliderArea.setRange(1, 100000)
        self.dialog.sliderPerimeter.setRange(1, 100000)
        self.dialog.sliderElongation.setRange(10, 100)  # shown /10
        self.dialog.sliderRectangularity.setRange(10, 100)  # shown /100
    ...
    def filter_vectors_by_metrics(self):
        try:
            ...
            if hasattr(self.dialog, 'labelMinArea'):
                self.dialog.labelMinArea.setText(f"Min area: {min_area:,}")
            if hasattr(self.dialog, 'labelMinPerimeter'):
                self.dialog.labelMinPerimeter.setText(
                    f"Min perimeter: {min_per:,}")
            if hasattr(self.dialog, 'labelMaxElongation'):
                self.dialog.labelMaxElongation.setText(
                    f"Max elongation: {max_el:.1f}")
            if hasattr(self.dialog, 'labelMinRectangularity'):
                self.dialog.labelMinRectangularity.setText(
                    f"Min rectang: {min_rect:.2f}")
    ...
    def open_filtered_preview(self):
        ...
        # ---- Sliders / Spins ----
        def metric_slider(label, minv, maxv, step, decimals=0, scale=1.0):
            """Helper para criar um par slider/spinbox com rótulo"""
            box = QHBoxLayout()
            lbl = QLabel(label)
            sld = QSlider(Qt.Horizontal)
            sld.setRange(minv, maxv)
            sld.setSingleStep(step)
            spn = QDoubleSpinBox() if decimals > 0 else QSpinBox()
            if decimals > 0:
                spn.setDecimals(decimals)
            spn.setRange(minv/scale, maxv/scale)
            spn.setSingleStep(step/scale)
            spn.setValue(minv/scale)
            box.addWidget(lbl)
            box.addWidget(sld)
            box.addWidget(spn)
            return box, sld, spn
